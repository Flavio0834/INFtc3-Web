...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/time"):
            ...
            self.path = "css/" + self.path.split("/")[-1]
            super().do_GET()
    ...
    def send_volcans(self):
        ...
        self.send(
            db.get_list_of_volcanos(), [("Content-type", "text/plain;charset=utf-8")]
        )
    ...
    def send_volcan(self, name):
        ...
        if infos:
            infos = infos.split("\n")
            res = '<!DOCTYPE html>\n<html>\n\t<head>\n\t\t<link rel="stylesheet" href="TD2-s9.css">\n\t</head>\n\t<body>\n'
            res += "\t\t<h1>" + infos[1][6:] + "</h1>\n"
            res += "\t\t<ul>\n"
